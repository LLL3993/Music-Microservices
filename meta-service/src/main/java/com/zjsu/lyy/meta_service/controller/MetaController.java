...
public class MetaController {
	...
	@GetMapping
	public ResponseEntity<List<MetaResponse>> listAll(@RequestHeader(value = "X-User-Is-Admin", required = false) String isAdmin) {
		if (!"true".equalsIgnoreCase(String.valueOf(isAdmin))) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
		}
		return ResponseEntity.ok(metaService.listAll());
	}
